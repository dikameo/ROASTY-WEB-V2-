...
<a class="flex items-center gap-2 text-primary hover:opacity-80 transition-opacity" href="{{ route('home') }}">
...
<p class="animate-pulse">⏳ Memuat metode pembayaran...</p>
</div>
</div>
</section>
</div>
...
async function loadMidtransScript() {
    ...
    if (window.snap) {
        console.log('✓ Midtrans script sudah tersedia');
        return true;
    }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            script.onload = () => {
                ...
                const checkSnap = setInterval(() => {
                    ...
                    if (window.snap && typeof window.snap.pay === 'function') {
                        ...
                        snapReady = true;
                        console.log('✓ window.snap siap digunakan');
                        resolve(true);
                    } else if (attempts > 20) {
                        clearInterval(checkSnap);
                        console.warn('⚠️ window.snap belum siap setelah 2 detik');
            ...
            console.error('❌ Error setting up Midtrans script:', err.message);
            reject(err);
        }
    });
}
...
function setupPaymentButton() {
    ...
    payBtn.addEventListener('click', async function(e) {
        ...
        try {
            ...
            saveLog('🔒 Button disabled, processing payment...');

            // Get selected payment method
            const paymentMethod = document.querySelector('input[name="payment"]:checked')?.value || 'gopay';
            saveLog('💳 Selected payment method: ' + paymentMethod);
            ...
            saveLog('📤 Sending order to backend...');
            ...
            console.log('🎫 Snap token type:', typeof snapToken);
            console.log('🎫 Snap token value:', snapToken);
            console.log('🎫 Snap token length:', snapToken ? snapToken.length : 0);

            if (snapToken) {
                console.log('🎫 Snap token received, opening Midtrans payment...');
                ...
                if (!window.snap) {
                    console.log('⏳ Menunggu Midtrans script di-load...');
                    try {
                        const scriptLoaded = await loadMidtransScript();
                        console.log('✅ Midtrans script loading resolved');
                ...
                console.log('🔍 Final check - window.snap:', window.snap);

                if (!window.snap || typeof window.snap.pay !== 'function') {
                    console.error('❌ window.snap tidak tersedia atau snap.pay bukan function');
                    alert('Payment gateway Midtrans tidak siap. Silakan:\n1. Refresh halaman\n2. Cek koneksi internet\n3. Coba browser lain');
                    throw new Error('Midtrans Snap tidak tersedia untuk digunakan');
                }

                console.log('✅ window.snap tersedia dan siap untuk pay()');
                console.log('   Token preview:', snapToken.substring(0, 20) + '...');

                try {
                    saveLog('🚀 Memanggil snap.pay() dengan token: ' + snapToken.substring(0, 15) + '...');
                    console.log('🚀 Calling snap.pay() dengan callback');

                    // Call snap.pay dengan callback untuk handle hasil
                    window.snap.pay(snapToken, {
                        onSuccess: function(result) {
                            saveLog('✅ Pembayaran sukses!');
                        ...
                        onPending: function(result) {
                            saveLog('⏳ Pembayaran pending...');
                            console.log('⏳ Payment pending:', result);
                            alert('Pembayaran sedang diproses. Silakan tunggu.');
                        },
                        onError: function(result) {
                            saveLog('❌ Pembayaran error: ' + result.status_message);
                        ...
                        onClose: function() {
                            saveLog('❌ Pop-up ditutup oleh user');
                    ...
                    saveLog('✅ snap.pay() berhasil dipanggil');
                    console.log('✅ snap.pay() called with callbacks');

                } catch (payError) {
                    saveLog('❌ Error calling snap.pay(): ' + payError.message);
                    console.error('❌ Error calling snap.pay():', payError);
                ...
                saveLog('⚠️ No snap token in response, assuming payment pending');
                console.warn('⚠️ No snap token in response, assuming payment pending');
            ...
            saveLog('❌ PAYMENT ERROR: ' + err.message);
